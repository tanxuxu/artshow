...
var ge1doot1 = ge1doot1 || {};
ge1doot1.transform3D = {};

//创建文字
ge1doot1.transform3D.text = function(){
	this.text = '详情';
	this.ctx = ge1doot1.screen.ctx;
}

ge1doot1.transform3D.text.prototype.drawText = function(x, y){
	this.ctx.fillStyle = '#f00';
	this.ctx.fillText(this.text, x, y);
}

/* ==== 绘制多边形 ==== */
ge1doot1.transform3D.drawPoly = function () {
	this.ctx.beginPath();
...
/* =============== 创建相机 ================= */
ge1doot1.transform3D.Camera = function (setup, func) {
	ge1doot1.camera = this;
	this.x  = 0;
	this.y  = 0;
	this.z  = 0;
	this.rx = 0;
...
/* ==== 相机缓动 ==== */
ge1doot1.transform3D.Camera.prototype.ease = function (target, value) {
...
/* ==== 移动旋转相机 ==== */
ge1doot1.transform3D.Camera.prototype.move = function () {
	// ---- run custom function ----
	this.cmov && this.cmov();
	// ---- translations ----
...
/* =============== 绘制点 ================= */
ge1doot1.transform3D.Point = function (x, y, z, tx, ty) {
	this.x = x;
	this.y = y;
	this.z = z;
	this.tx = tx || 0;
...
/* ==== 创建透视 投影 ==== */
ge1doot1.transform3D.Point.prototype.projection = function () {
...
/* ==== 三角形构造函数 ==== */
ge1doot1.transform3D.Triangle = function (parent, p0, p1, p2) {
	this.ctx = parent.ctx;
...
/* ==== 绘制三角形 ==== */
ge1doot1.transform3D.Triangle.prototype.draw = function () {
...
/* ===================== 图片构造函数 ========================== */
ge1doot1.transform3D.Image = function (parent, name, imgSrc, lev, callback) {
	this.name = name; //名称
	this.parent        = parent;
	this.points        = [];
	this.triangles     = [];
	this.ctx           = ge1doot1.screen.ctx;
	this.pointer       = ge1doot1.pointer;
	this.texture       = imgSrc   ;//new Image();
	...
	this.visible       = false;
	this.t             = false;
	this.text = new ge1doot1.transform3D.text();
	if (!ge1doot1.transform3D.Point.prototype.scr) {
		ge1doot1.transform3D.Point.prototype.scr    = ge1doot1.screen;
		ge1doot1.transform3D.Point.prototype.camera = ge1doot1.camera;
	}
}
/* ==== 图片中绘制多边形 ==== */
ge1doot1.transform3D.Image.prototype.drawPoly = ge1doot1.transform3D.drawPoly;
/* ==== change tessellation level prototype ==== */
ge1doot1.transform3D.Image.prototype.setLevel = function (level) {
...
/* ==== loading prototype ==== */
ge1doot1.transform3D.Image.prototype.loading = function () {
...
ge1doot1.transform3D.Image.prototype.subdivise = function (p0, p1) {
	return {
		x:  (p1.x + p0.x)   * 0.5,
		y:  (p1.y + p0.y)   * 0.5,
		z:  (p1.z + p0.z)   * 0.5,
...
/* ==== triangulation ==== */
ge1doot1.transform3D.Image.prototype.triangulate = function (p0, p1, p2, level) {
	level--;
	if (level === 0) {
		// final triangle
		this.t = new ge1doot1.transform3D.Triangle(this, p0, p1, p2);
		...
		// ---- insert new points ----
		this.points.push(p01 = new ge1doot1.transform3D.Point(p01.x, p01.y, p01.z, p01.tx, p01.ty));
		this.points.push(p12 = new ge1doot1.transform3D.Point(p12.x, p12.y, p12.z, p12.tx, p12.ty));
		this.points.push(p20 = new ge1doot1.transform3D.Point(p20.x, p20.y, p20.z, p20.tx, p20.ty));
...
/* ==== transform prototype ==== */
ge1doot1.transform3D.Image.prototype.transform3D = function (backfaceTest) {
	if (this.isLoading) {
		// ---- image is loading ----
		this.loading();
		return false;
...
/* ==== draw prototype ==== */
ge1doot1.transform3D.Image.prototype.draw = function () {
	if (!this.isLoading) {
		// ---- draw triangles ----
		for (
			var i = 0;
...
ge1doot1.transform3D.Image.prototype.isPointerInside = function (x, y) {
